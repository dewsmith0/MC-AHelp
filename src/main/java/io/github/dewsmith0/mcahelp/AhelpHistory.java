...
public class AhelpHistory {
    ...
    public static void addLog(OfflinePlayer player, Player sender, String message) {
        try (Connection connection = plugin.getDatabase().getConnection()) {
            String sql = """
                    INSERT INTO ahelp_logs (
                        player_uuid, sender_uuid, message
                    ) VALUES (?, ?, ?);
                    """;
    ...
    public static ArrayList<AhelpEntry> getLogs(OfflinePlayer player, int limit) {
        try (Connection connection = plugin.getDatabase().getConnection()){
            PreparedStatement statement = connection.prepareStatement("""
                    SELECT * FROM ahelp_logs WHERE player_uuid = ? LIMIT ?;
                    """);
            statement.setString(1, player.getUniqueId().toString());
            ...
            while(result.next()) {
                ...
                AhelpEntry entry = new AhelpEntry(playerUuid, senderUuid, message, timestamp);
                output.add(entry);
    ...
    public record AhelpEntry(UUID playerUuid, UUID senderUuid, String message, Date timestamp) {
        //public AhelpEntry {
//
  //      }
    }
